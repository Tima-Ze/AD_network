...
sourceCpp("wTO_correlation_snakemake.cpp")
                      ...
                      ID = NULL) 
{
  ...
  if (!is.null(DIM_Overlap)) {
    message(paste("There are", DIM_Overlap, "overlapping nodes,", 
  ...
  wtomelt0 = CorrelationOverlap_edit(Data = Data, Overlap = Overlap, 
                                     method = method) %>% wTO_edit(., sign)
  `%>%` <- magrittr::`%>%`
  . <- NULL
...
#                                                                                     sign)
      res = CorrelationOverlap_edit(Data = Data_boot, 
                                    Overlap = Overlap, method = method) %>% wTO_edit(., 
                                                                                     sign)
...
wTO.Complete_edit = function(k = 1 ,n = 100, Data , Overlap = row.names(Data),
  ...
  if ( k == 1){
    ...
    while ( reps_rest  > 0){
      ...
      # K = 1:n
      OUTPUT = lapply(K, wTO.aux.each_edit, Data= Data,
    ...
    while ( reps_rest  > 0){
      ...
      OUTPUT = parallel::clusterApply(cl, K, wTO.aux.each_edit , Data= Data,
  ...
  if(plot == T){
    ...
    graphics::axis(1, las = 1, cex.axis = 0.8, col = "steelblue",
    ...
    PLOT <- function(wTO_value){
      ...
      graphics::legend(c(0.9,max(wTO_value$relstar)), c("wTO - Data set",
                       ...
                       lty = 1, col = c("violet",
                                        ...
                                        "yellow", "orange", "red"), bty = "n", cex = 0.8  )
